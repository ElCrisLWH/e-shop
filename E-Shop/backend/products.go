...
func Handler(w http.ResponseWriter, r *http.Request) {
	...
	//Convert the product variable declared above into a json
	...
	_, err = w.Write(b)
	if err != nil {
		http.Error(w, "Internal Server Error Write", http.StatusInternalServerError)
		return
	}
}

func main() {
	//Start local web server at 8080 that handles all requests with Handler and specify the port
